...
def main(train, test):
    ...
    train_params = train.params_linear
    train_data = train.data
    train_save = train.save_linear
    ...
    os.path.join(modelpath, 
        f"Iter_{train.params.epoch}_{train.save.model_name}_eth_ssl.pt"), 
    ...
    train_data, train_folder = ctools.readfolder(
                        train.data, 
                        [test.person], 
                        reverse=True
                    )
    ...
    train_savepath = os.path.join(train_save.metapath, train_save.folder, f'checkpoint/{testname}')
    ...
    regressorpath = os.path.join(train.save_linear.metapath,
                             train.save_linear.folder, f'checkpoint/{testname}')
    ...
    for saveiter in range(begin, end+step, step):
        ...
        regressor_statedict = torch.load(
            os.path.join(regressorpath, 
                f"Iter_{saveiter}_{train.save_linear.model_name}.pt"), 
            map_location={f"cuda:{train.device}": f"cuda:{test.device}"}
        )
